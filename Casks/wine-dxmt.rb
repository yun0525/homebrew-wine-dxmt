cask "wine-dxmt" do
  ...
  preflight_steps do
    ...
    unless system_command("/usr/bin/xcode-select", args: ["-p"], print_stderr: false).exit_status.zero?
      ::Kernel.puts "Installing Xcode Command Line Tools..."
      system_command "/usr/bin/xcode-select", args: ["--install"]
      ::Kernel.puts "Please complete the Xcode CLI Tools installation, then re-run: brew install --cask wine-dxmt"
      ::Kernel.raise "Xcode Command Line Tools required. Please install and retry."
    end
    ...
    unless system_command("/usr/bin/arch", args: ["-x86_64", "/usr/bin/true"], print_stderr: false).exit_status.zero?
      ::Kernel.puts "Installing Rosetta 2..."
    ...
    unless File.exist?("/usr/local/bin/brew")
      ::Kernel.puts "Installing x86_64 Homebrew..."
  ...
  postflight_steps do
    ...
    unless File.exist?("#{wine_dir}/bin/wine")
      ::Kernel.puts "Installing Wine Staging 11.8_mm1..."
    ...
    # (NVIDIA stubs).
    ::Kernel.puts "Downloading DXMT v0.80 prebuilt..."
    ...
    ::Kernel.puts "Applying DXMT v0.80 UE5.6-patched binaries..."
    ...
    unless File.exist?("/usr/local/lib/gstreamer-1.0")
      ::Kernel.puts "Installing x86_64 GStreamer..."
